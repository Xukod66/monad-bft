...
impl std::fmt::Debug for UdpSocketHandle {
...
impl UdpDataplane {
    pub fn socket_handles(&mut self) -> &mut [UdpSocketHandle] {
        &mut self.socket_handles
    }
    ...
    pub fn take_socket_by_id(&mut self, socket_id: usize) -> Option<UdpSocketHandle> {
        self.socket_handles
            .iter()
            .position(|h| h.writer.socket_id == socket_id)
            .map(|idx| self.socket_handles.swap_remove(idx))
    }
}
...
impl std::fmt::Debug for TcpSocketHandle {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("TcpSocketHandle").finish()
    }
}
